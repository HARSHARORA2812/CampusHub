...
// Helper function to extract college ID from email
const extractCollegeId = (email) => {
  // Extract text between @ and first dot (e.g., stanford from @stanford.edu)
...
router.post('/register', validateRegister, async (req, res) => {
  try {
    ...
    // Extract college_id from email
    const college_id = extractCollegeId(email);

    // SECURITY: All new registrations are forced to "student" role
    // Only officials (college_management, college_admin, main_admin) can promote users to higher roles
    const user = new User({
      id: uuidv4(),
      email,
      full_name,
      role: 'student',  // Force student role for all new registrations
      college_id,  // Set college identifier
      bio: bio || '',
      interests: interests || [],
